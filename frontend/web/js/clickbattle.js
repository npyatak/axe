...
function create() {
    ...
    endGameSetTimeoutId = setTimeout(function () {
        ...
        $('#clickbattleresult-client_score').val(score);
        $('#clickbattleresult-clicks').val(JSON.stringify(clicks));
        $('#clickbattleresult-targets').val(JSON.stringify(targetsResult));

        if($('#shootingresult-recaptcha-recaptcha').length == 0) {
            $('#score_form').submit();
        }
